 ...
 * その総和を出力
 */
const frequency_score_one_name = (name, whiteList) => {
    /**スコア */
    let score = 0;
    /**ホワイトリストから計算した頻度 */
...
/**名前リストから頻度スコアリストを出力 */
export const frequency_score = (nameList) => {
    let scores = [];
...
/**スコア最大のポケモンを出力 */
export const frequency_best_score = (nameList) => {
...
const addBlackCharList = (blackCharList, postedChar) => {
...
const addYellowCharList = (yellowCharList, postedChar) => {
...
const addGreenCharList = (greenCharList, postedChar) => {
    postedChar.forEach((value, index) => {
...
/**文字の評価リストを作成 */
export const addCharList = (postedCharDataList, postedChar) => {
...
/**ホワイトリストを変更 */
export const changeWhiteList = (whiteList, postedCharDataList) => {
    const greenList = postedCharDataList.green;
    const yellowList = postedCharDataList.yellow;
    const yellowCharUnique = uniq(yellowList.map((value) => value.char));
    ...
    if (yellowList.toString()) {
        ...
        const yellowReList = new Array(5).fill("");
        yellowList.forEach((value) => {
            yellowReList[value.position] += value.char;
        });
        const yellowRe = new RegExp(yellowReList
            .map((value) => "[^" + value + "]")
            .toString()
            .replaceAll(",", ""));
