...
import { ArrayFiveValues, PostedCharType, PostedDataType } from "./type.js";
...
const oneGame = async () => {
	...
	/**答えとなる可能性がない文字 */
	const blackCharList: string[] = [];
	/**答えに含まれるが場所が異なる文字 */
	const yellowCharList: { char: string; position: number }[] = [];
	/***場所も一致している文字 */
	const greenCharList = <ArrayFiveValues<string>>new Array(5).fill("");
	/**文字の評価リスト */
	...
	// 初期化ここまで
	/**１ターン */
	const oneTurn = async (): Promise<any> => {
		/**ローカルストレージから取得したデータ */
		...
		if (isEnd(currentTurn)) {
			await sleep(1000)
			const nextButton = <HTMLElement>document.querySelector("#endless > div > div.keyboard.mx-auto.mt-4.pt-md-4.mb-6 > div.row.my-2.mx-1.align-content-end > button.ml-2.font-weight-bold.v-btn.v-btn--has-bg.theme--dark.elevation-0.v-size--default.green");
			nextButton.click();
			return oneGame();
		}
		// ターン終了
